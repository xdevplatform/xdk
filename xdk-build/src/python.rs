...
use colored::*;
use std::fs; // Add fs for directory removal

/// Generator for Python SDKs.
#[derive(Default)]
...
impl LanguageGenerator for PythonGenerator {
    ...
    fn generate(&self, openapi: &OpenApi, output_dir: &Path) -> Result<()> {
        ...
        log_info!("Creating Python virtual environment at: {}", venv_path.display().to_string().magenta());
        ...
        log_info!("Installing black and libcst using {}", venv_python_path.display().to_string().magenta());
        let mut install_deps_cmd = Command::new(&venv_python_path);
        install_deps_cmd.arg("-m").arg("pip").arg("install").arg("black");
        ...
        Self::run_black(output_dir, &venv_black_path)?;
        ...
        Self::run_formatter(output_dir, &venv_python_path, script_path)?;

        // 5. Clean up the virtual environment
        log_info!("Removing formatting virtual environment: {}", venv_path.display().to_string().magenta());
...
// Run black using the provided virtual environment executable
    fn run_black(output_dir: &Path, venv_black_path: &Path) -> Result<()> {
        log_info!("Formatting code with black using {}", venv_black_path.display().to_string().magenta());
    ...
    fn run_formatter(output_dir: &Path, venv_python_path: &Path, script_path: &Path) -> Result<()> {
        log_info!("Running formatter script {} using {}", script_path.display().to_string().magenta(), venv_python_path.display().to_string().magenta());
