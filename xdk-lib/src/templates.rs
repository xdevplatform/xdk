...
fn get_header_for_file(template_name: &str, output_path: Option<&str>) -> String {
    ...
    let (comment_start, comment_line, comment_end) = if file_type == "python" || file_type == "toml" || file_type == "ignore" {
        ("", "#", "")
        ...
        return String::new();
    } else if template_name == "readme" 
        || template_name.ends_with("_md") {
        // Markdown files use HTML comments
        ("<!--", "", "-->")
    } else if template_name == "npmignore" 
        || template_name.ends_with("ignore") {
    ...
    if comment_start.is_empty() && comment_end.is_empty() {
        ...
        format!(
            "{} {}\n{} {}\n{} {}",
            comment_line, notice,
            comment_line, details,
            comment_line, warning
        )
    } else {
...
mod tests {
    ...
    fn test_header_not_added_twice() {
        ...
        assert_eq!(count, 2, "Header should appear exactly twice when added twice");
    }
