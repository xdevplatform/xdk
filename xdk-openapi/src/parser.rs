        ...
        OpenApiContextGuard::with_context_mut(|ctx| {
            if let Some(schemas) = components.get("schemas").and_then(|s| s.as_mapping()) {
                for (name, schema_val) in schemas {
                    if let Some(name_str) = name.as_str() {
                        match serde_yaml::from_value::<crate::components::Schema>(schema_val.clone()) {
                            Ok(schema) => ctx.add_schema(name_str.to_string(), schema),
                            Err(e) => eprintln!("Warning: Failed to parse schema {}: {}", name_str, e),
                        }
                    }
                }
            }
            if let Some(parameters) = components.get("parameters").and_then(|p| p.as_mapping()) {
                for (name, param_val) in parameters {
                    if let Some(name_str) = name.as_str() {
                         match serde_yaml::from_value::<crate::components::Parameter>(param_val.clone()) {
                            Ok(param) => ctx.add_parameter(name_str.to_string(), param),
                            Err(e) => eprintln!("Warning: Failed to parse parameter {}: {}", name_str, e),
                        }
                    }
                }
            }
                ...
                for (name, resp_val) in responses {
                    if let Some(name_str) = name.as_str() {
                         match serde_yaml::from_value::<crate::core::Response>(resp_val.clone()) {
                            ...
                            Err(e) => eprintln!("Warning: Failed to parse response {}: {}", name_str, e),
                        }
                    }
                }
            }
             if let Some(request_bodies) = components.get("requestBodies").and_then(|rb| rb.as_mapping()) {
                for (name, rb_val) in request_bodies {
                    if let Some(name_str) = name.as_str() {
                         match serde_yaml::from_value::<crate::core::RequestBody>(rb_val.clone()) {
                            ...
                            Err(e) => eprintln!("Warning: Failed to parse requestBody {}: {}", name_str, e),
                        }
                    }
                }
            }
            if let Some(security_schemes) = components.get("securitySchemes").and_then(|ss| ss.as_mapping()) {
                for (name, ss_val) in security_schemes {
                    if let Some(name_str) = name.as_str() {
                         match serde_yaml::from_value::<crate::components::SecurityScheme>(ss_val.clone()) {
                            Ok(ss) => ctx.add_security_scheme(name_str.to_string(), ss),
                            Err(e) => eprintln!("Warning: Failed to parse securityScheme {}: {}", name_str, e),
                        }
                    }
                }
            }
        }).ok_or_else(|| OpenApiError::InternalError("Context not available during component pre-loading in parse_yaml".to_string()))?;
    }
    ...
    if let Some(components) = value.get("components") {
        OpenApiContextGuard::with_context_mut(|ctx| {
            ...
            if let Some(parameters) = components.get("parameters").and_then(|p| p.as_object()) {
             ...
             if let Some(request_bodies) = components.get("requestBodies").and_then(|rb| rb.as_object()) {
                for (name, rb_val) in request_bodies {
            ...
            if let Some(security_schemes) = components.get("securitySchemes").and_then(|ss| ss.as_object()) {
                for (name, ss_val) in security_schemes {
                     match serde_json::from_value::<crate::components::SecurityScheme>(ss_val.clone()) {
                        Ok(ss) => ctx.add_security_scheme(name.clone(), ss),
                        Err(e) => eprintln!("Warning: Failed to parse securityScheme {}: {}", name, e),
                    }
                }
            }
        }).ok_or_else(|| OpenApiError::InternalError("Context not available during component pre-loading in parse_json".to_string()))?;
    }
