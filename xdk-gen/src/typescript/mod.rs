...
mod tests {
    ...
    fn test_version_in_package_json() {
        ...
        assert!(
            contents.contains("\"version\": \"1.2.3-test\""),
    ...
    fn test_user_agent_in_client() {
        ...
        let contents = fs::read_to_string(&client_path).expect("Failed to read client.ts");
        assert!(
            contents.contains("'User-Agent': 'xdk-typescript/0.1.1-beta'"),
    ...
    fn test_user_agent_not_in_config_interface() {
        ...
        assert!(
            !config_section.contains("userAgent"),
