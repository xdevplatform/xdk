...
class TestAccountActivityGeneric:
    ...
    def test_client_has_expected_base_functionality(self):
        ...
        public_methods = [name for name in dir(self.account_activity_client) 
                         if not name.startswith('_') and callable(getattr(self.account_activity_client, name))]
        assert len(public_methods) > 0, f"AccountActivityClient should have at least one public method"
    
    def test_client_method_signatures_are_valid(self):
        """Test that all client methods have valid Python signatures."""
        public_methods = [name for name in dir(AccountActivityClient) 
                         if not name.startswith('_') and callable(getattr(AccountActivityClient, name))]
