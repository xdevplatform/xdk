...
class TestComplianceGeneric:
    ...
    def test_client_has_expected_base_functionality(self):
        ...
        public_methods = [name for name in dir(self.compliance_client) 
                         if not name.startswith('_') and callable(getattr(self.compliance_client, name))]
        assert len(public_methods) > 0, f"ComplianceClient should have at least one public method"
