...
class TestCommunitiesGeneric:
    ...
    def test_client_has_expected_base_functionality(self):
        ...
        public_methods = [name for name in dir(self.communities_client) 
                         if not name.startswith('_') and callable(getattr(self.communities_client, name))]
        assert len(public_methods) > 0, f"CommunitiesClient should have at least one public method"
    
    def test_client_method_signatures_are_valid(self):
        """Test that all client methods have valid Python signatures."""
        public_methods = [name for name in dir(CommunitiesClient) 
                         if not name.startswith('_') and callable(getattr(CommunitiesClient, name))]
