...
class TestDirectMessagesStructure:
    ...
    def test_get_dm_events_exists(self):
        ...
        assert (
            method is not None
        ), f"Method get_dm_events does not exist on DirectMessagesClient"
        # Check method is callable
        ...
        for required_param in required_params:
            assert (
                required_param in params
            ), f"Required parameter '{required_param}' missing from get_dm_events"
        # Check optional parameters have defaults (excluding 'self')
        optional_params = [
            "max_results",
            "pagination_token",
    ...
    def test_get_dm_events_by_id_exists(self):
        ...
        assert (
            method is not None
        ), f"Method get_dm_events_by_id does not exist on DirectMessagesClient"
        # Check method is callable
        ...
        assert (
            len(params) >= 1
        ), f"get_dm_events_by_id should have at least 'self' parameter"
        assert (
            params[0] == "self"
        ), f"First parameter should be 'self', got '{params[0]}'"
        # Check required parameters exist (excluding 'self')
        ...
        for required_param in required_params:
            assert (
                required_param in params
            ), f"Required parameter '{required_param}' missing from get_dm_events_by_id"
        # Check optional parameters have defaults (excluding 'self')
        optional_params = []
        for optional_param in optional_params:
            if optional_param in params:
    ...
    def test_no_unexpected_public_methods(self):
        ...
        if unexpected_methods:
            print(
                f"Warning: Unexpected methods found on DirectMessagesClient: {unexpected_methods}"
            )
