...
def _validate_paginatable_method(method: Callable) -> None:
    ...
    # Check if method has pagination_token or max_results parameters
    has_pagination_token = "pagination_token" in params
    has_max_results = "max_results" in params
    if not (has_pagination_token or has_max_results):
        raise PaginationError(
            f"Method '{method.__name__}' does not support pagination. "
            f"Paginatable methods must accept 'pagination_token' and/or 'max_results' parameters. "
...
class Cursor(Generic[ResponseType]):
    ...
    Example usage:
        # Get individual items - cursor infers the item type
        cursor = Cursor(client.users.get_users, max_results=100)
        for user in cursor.items(250):  # user is properly typed
            print(user.name)

        # Get pages - returns the same type as the original method
        for page in cursor.pages(5):  # page is GetUsersResponse type
            print(f"Got {len(page.data)} tweets")
    """


    def __init__(self, method: PaginatableMethod[ResponseType], **kwargs: Any):
        """
        Initialize the cursor.
        Args:
            method: The API method to call for each page (must support pagination)
            **kwargs: Arguments to pass to the API method (excluding pagination params)
        Raises:
...
def cursor(
    method: PaginatableMethod[ResponseType], **kwargs: Any
) -> Cursor[ResponseType]: ...
...
def cursor(
    ...
    Example:
        ...
        users_cursor = cursor(client.users.get_users, max_results=100)
        # page is typed as GetUsersResponse
        for page in users_cursor.pages(5):
            print(len(page.data))
    """
    return Cursor(method, **kwargs)
...
class _PageIterator(Generic[ResponseType], Iterator[ResponseType]):
    ...
    def __next__(self) -> ResponseType:
        ...
        # Make the API call
        response = self.cursor.method(**params)
        # Extract next token
